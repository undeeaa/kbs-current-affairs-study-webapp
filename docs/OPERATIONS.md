...
1. 현재 회차를 `재시험 종료`까지 진행합니다.
2. 관리 화면의 `다음 회차를 준비할까요?` 영역에서 회차 제목을 확인합니다.
3. `다음 회차 준비하기`를 누릅니다.
4. 웹앱이 이전 회차에서 사용하지 않은 유효한 문제 20개를 `items`의 시트 순서대로 선택합니다.
5. 새 `Rounds` 행과 `RoundQuestions` 20행이 생성되고 상태는 `WAITING`이 됩니다.
6. 참가자가 대기 화면에 들어온 것을 확인한 뒤 `1차 시험 시작`을 누릅니다.

사용하지 않은 문제는 ID, 문제 본문, 대표 정답이 모두 있어야 합니다. 20개보다 적으면 회차를 만들지 않고 부족한 개수를 안내합니다.
...
참가자 화면은 문제와 결과 전체를 반복해서 가져오지 않고 가벼운 `status` 응답만 확인합니다. 대기·정답 확인은 응답 완료 후 약 3초, 시험 중은 약 5초, 종료 후는 약 20초 간격이며 숨겨진 탭에서는 확인을 멈춥니다.
...
Apps Script를 갱신할 때는 프런트보다 먼저 새 버전을 배포하고 `/exec?action=status`에서 `roundId`, `status`만 반환되는지 확인합니다. 이전 백엔드와 연결되면 프런트는 자동으로 기존 조회 방식으로 전환합니다.
